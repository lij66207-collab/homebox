...
让 OpenClaw（中文昵称"龙虾"，前身为 Clawdbot / Moltbot）通过 Homebox REST API 帮你全面管理家庭库存：录入（含批量录入）、查询临期、移动位置、修改数量/标签/价格等信息、删除（含批量删除）——网页 UI 上能做的物品管理操作，都可以交给龙虾完成。
...
- "帮我把这几样都录进储物间：5 号电池 8 节、垃圾袋 2 卷、纸巾 6 包"
- "我把采购清单发你，照着批量入库"
...
OpenClaw 会自动完成：解析位置/物品名称 → 必要时创建位置 → 执行创建/更新/删除 → 用一句话向你确认。删除和批量删除属于破坏性操作，龙虾会先把受影响的物品清单列给你，得到明确同意后才会执行。
